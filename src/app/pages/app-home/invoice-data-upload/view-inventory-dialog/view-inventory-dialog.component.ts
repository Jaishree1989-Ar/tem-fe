...
@Component({
  ...
  styleUrl: './view-inventory-dialog.component.less'
})
export class ViewInventoryDialogComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<ViewInventoryDialogComponent>,
  ...
  getInventoryById() {
    const url = `inventory-history/review/${this.data.batchId}`;
    this.apiService.get<ApiResponse<any>>(url).subscribe({
      next: (res) => {
        this.inventoryDetails = res.data;
        this.dataSource.data = this.inventoryDetails.inventoryRecords;
        if (this.inventoryDetails.inventoryRecords?.length > 0) {
          this.selectRecord(this.inventoryDetails.inventoryRecords[0]);
        }
      },
  ...
  getApprovedInventoryById() {
    const url = `inventory-history/batch/${this.data.batchId}/approved`;
    this.apiService.get<ApiResponse<any>>(url).subscribe({
      next: (res) => {
        this.inventoryDetails = res.data;
        this.dataSource.data = this.inventoryDetails.inventoryRecords;
  ...
  onApprove(): void {
    const dialogRef = this.dialog.open(ConfirmationDialogComponent, {
      ...
      data: {
        ...
        message: 'Are you sure you want to approve this inventory details?',
        confirmText: 'Approve',
        cancelText: 'Cancel'
      }
    });
  ...
  onReject(): void {
    const dialogRef = this.dialog.open(ConfirmationDialogComponent, {
      ...
      data: {
        ...
        message: 'Are you sure you want to reject this inventory details?',
        confirmText: 'Reject',
        cancelText: 'Cancel'
      }
    });
  ...
  private sendReviewAction(action: 'APPROVE' | 'REJECT', reason?: string): void {
    ...
    const url = `inventory-history/review/action`;
    this.apiService.post<any>(url, payload).subscribe({
      next: (res) => {
        if (action === 'APPROVE') {
          this.snackbarService.open('success', SNACKBAR_MESSAGES.INVENTORY_REVIEW_APPROVED);
        } else if (action === 'REJECT') {
          this.snackbarService.open('success', SNACKBAR_MESSAGES.INVENTORY_REVIEW_REJECTED);
        }
        this.dialogRef.close({ updated: true, action });
      },
      error: (err) => {
        this.snackbarService.open('error', SNACKBAR_MESSAGES.INVENTORY_REVIEW_INTERNAL_ERROR);
